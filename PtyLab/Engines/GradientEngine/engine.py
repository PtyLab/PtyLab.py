...
class GradientEngine(BaseEngine):
    ...
    Adam step. Only one diffraction frame's autograd graph is retained at a time.
    ``reconstruction.error`` records the pre-update sum of squared amplitude
    residuals divided by total measured intensity (not the PIE error metric).
    ...
    def validateSettings(self):
        """Validate the default model; subclasses can extend supportedPropagators."""
    ...
    def propagate(self, object_patch, probe):
        """Form the exit wave and pass it to the propagation component."""
    ...
    def prepareReconstruction(self):
        """Create fresh tensors and optimizer from the current NumPy estimates."""
    ...
    def computeLoss(self, detectorWave, measuredAmplitude):
        """Delegate to lossFunction; frame losses are summed over the scan."""
        return self.lossFunction(detectorWave, measuredAmplitude, self.totalPower)

    def regularizationLoss(self):
        """Sum configured penalties once per iteration, or return None."""
    ...
    def runIteration(self, iteration):
        """Accumulate frame gradients, add regularization, then take one step."""
        self.setPositionOrder()
        self.optimizer.zero_grad(set_to_none=True)
        total_loss = torch.zeros((), device=self.device)
        for index in self.positionIndices:
            detector_wave = self.forward(index)
            loss = self.computeLoss(detector_wave, self.measuredAmplitudes[index])
            loss.backward()
        ...
        self.reconstruction.Iestimated = (
            detector_wave.detach().abs().square().cpu().numpy().copy()
        )
        self.reconstruction.Imeasured = (
            self.measuredAmplitudes[index].square().cpu().numpy().copy()
        )
        return total_loss.item()

    def reconstruct(self, experimentalData=None):
        """Run from the current estimates with a fresh optimizer on every call."""
