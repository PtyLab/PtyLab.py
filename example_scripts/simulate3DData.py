...
simuData.npsm = 1
simuData.nslice = 2
if simuData.nslice>1:
    simuData.dz =  1e-3 #1.5*simuData.wavelength
...
simuData.object[...,0,:,:] = obj
t = (1 + np.sign(np.sin(-np.round(b/4) * theta + 2*np.pi * (rho/d)**2)))/2
# phaseFun = np.exp(1.j * np.arctan2(simuData.Yo, simuData.Xo))
phaseFun = 1
# phaseFun = np.exp(1.j*( 2* theta + 2*np.pi * (rho/d)**2))
t = t*circ(simuData.Xo, simuData.Yo, simuData.Lo)*(1-circ(simuData.Xo, simuData.Yo, 120*simuData.dxo))*phaseFun\
    +circ(simuData.Xo, simuData.Yo, 50*simuData.dxo)
obj = convolve2d(t, gaussian2D(5, 3), mode='same')  # smooth edges
# obj_phase = np.exp(1.j*2*np.pi/simuData.wavelength*(simuData.Xo**2+simuData.Yo**2)*20)
obj = obj*phaseFun
simuData.object[...,1,:,:] = np.roll(np.rot90(obj),70)
plt.figure(figsize=(5,5), num=20)
ax = plt.axes()
hsvplot(np.squeeze(simuData.object)[...,0,:,:], ax=ax)
ax.set_title('Object at probe side')
plt.show(block=False)
plt.figure(figsize=(5,5), num=21)
ax = plt.axes()
hsvplot(np.squeeze(simuData.object)[...,1,:,:], ax=ax)
ax.set_title('Object at detector side')
plt.show(block=False)
# generate positions
# generate non-uniform Fermat grid
# parameters
numPoints = 502   # number of points
radius = 200    # radius of final scan grid (in pixels)
...
ax1 = plt.axes()
hsvplot(np.squeeze(simuData.object[...,0,:,:]), ax=ax1)
ax1.plot(positions[0,1]+simuData.Np//2,positions[0,0]+simuData.Np//2,'.')
ax1.set_title('Object on probe side')
plt.show(block=False)

plt.figure(figsize=(5,5), num=34)
ax1 = plt.axes()
...
for loop in np.arange(simuData.numFrames):
    ...
    objectPatch = simuData.object[...,0, sy, sx].copy()
    # multiply each probe mode with object patch
    esw_temp = objectPatch*simuData.probe[...,0,:,:]
    simuData.probe[...,1,:,:] = aspw(esw_temp, simuData.dz,
                                        simuData.wavelength, simuData.Lp)[0]
    objectPatch = simuData.object[...,1, sy, sx].copy()
    optimizable.esw = objectPatch*simuData.probe[...,1,:,:]
    # generate diffraction data
    reconstructor.object2detector()
    # save data in ptychogram
    simuData.ptychogram[loop] = np.sum(abs(optimizable.ESW)**2, axis=(0, 1, 2))
...
if export_data:
    ...
    hf.create_dataset('dz', data=(simuData.dz,), dtype='f')
    hf.close()
