...
theta, rho = cart2pol(simuData.Xp, simuData.Yp)
theta[theta<0] = theta[theta<0] + 2*np.pi
simuData.probe = np.zeros((simuData.nlambda, 1, simuData.npsm, simuData.nslice, simuData.Np, simuData.Np),
                          dtype='complex128')
...
# multiply with quadratic phase and aperture
    ...
    simuData.probe[int2,...,:,:] = probe

    plt.figure(figsize=(5,5), num=15)
    ax1 = plt.subplot(121)
    hsvplot(pinhole, ax=ax1, pixelSize=simuData.dxp)
    ax1.set_title('complex pinhole')
    plt.subplot(122)
    plt.imshow(abs(pinhole)**2)
