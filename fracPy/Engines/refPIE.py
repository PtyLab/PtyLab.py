...
class refPIE(BaseEngine):
    ...
    def initializeReconstructionParams(self):
        ...
        temp_obj = self.reconstruction.object[0, 0, 0, 0, :, :]
        unwrapped_phase = unwrap_phase(xp.angle(temp_obj))

        self.reconstruction.object[0, 0, 0, 0, :, :] = unwrapped_phase - 1j * xp.log(xp.abs(temp_obj)) 
    ...
    def reconstruct(self):
        ...
        for loop in self.pbar:
            ...
            for positionLoop, positionIndex in enumerate(self.positionIndices):
                ...
                self.reconstruction.esw = xp.exp(1j * objectPatch) * self.reconstruction.probe
    ...
    def objectPatchUpdate(self, objectPatch: np.ndarray, DELTA: np.ndarray):
        ...
        absP2 = xp.abs(self.reconstruction.esw) ** 2
        ...
        if self.experimentalData.operationMode =='FPM':
            ...
            frac = xp.conj(1j * self.reconstruction.esw) / (self.alphaObject * Pmax + (1 - self.alphaObject) * absP2)
            # frac = 1j * xp.exp(1j * objectPatch) * self.reconstruction.probe.conj() / (self.alphaObject * Pmax + (1 - self.alphaObject) * absP2)
    ...
    def probeUpdate(self, objectPatch: np.ndarray, DELTA: np.ndarray):
        ...
        # find out which array module to use, numpy or cupy (or other...)
        xp = getArrayModule(objectPatch)
        absO2 = xp.abs(xp.exp(1j * objectPatch)) ** 2
        Omax = xp.max(xp.sum(absO2, axis=(0, 1, 2, 3)), axis=(-1, -2))
        frac = xp.exp(-1j * objectPatch.conj()) / (self.alphaProbe * Omax + (1-self.alphaProbe) * absO2)
