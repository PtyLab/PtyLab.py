...
class OPRP_storage:
    ...
    def center_probe(self, probe, index):
        dpos = np.array(ndimage.center_of_mass(abs(probe**2))) - np.array(probe.shape) / 2
    ...
    def _prepare_probes(self, single_probe, N_positions):
        ...
        self.probes = self.xp.zeros((self.N_positions, *self.new_probe_shape), dtype=np.complex64)
