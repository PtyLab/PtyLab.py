...
def metric_at(object_estimate, dz, dx, wavelength, ss=(slice(None, None), slice(None, None)),
              intensity_only=False, return_propagated=False, average_by_power=True, metric:str=TV)-> Union[
    ...
    if not isinstance(type(metric), type(callable)):
        try:
            ...
            raise KeyError(f'Could not map {metric} to a metric. Allowed keywords are: {[k for k in possible_metrics.keys()]}')
    sy, sx = ss
    ...
    OEs = []
    for z in dz:
        OE = op(aspw(xp.squeeze(OE_ff[...,sy,sx]), z=float(z),
