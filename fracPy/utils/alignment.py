...
def show_alignment(reconstruction: Reconstruction, data: ExperimentalData, params: Params,
    ...
    order = np.argsort(np.linalg.norm(reconstruction.positions - mean_pos, axis=-1))[:15]
    ptycho_ordered = data.ptychogram[order]
    # do one iteration of all of them

    from fracPy.Operators.Operators import object2detector, detector2object
    reconstruction.initializeObjectProbe()
    reconstruction.esw = reconstruction.probe
    # do all the propagators
    from fracPy.Operators.Operators import fresnelPropagator
